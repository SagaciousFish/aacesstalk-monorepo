...
class SQLSessionStorage(SessionStorage, SQLStorageBase):
    ...
    @classmethod
    async def __load_session_info_impl(
    ...
    async def get_dialogue(self) -> Dialogue:
        async with self.get_sessionmaker() as db:
            statement = (
                ...
                .where(DialogueMessageORM.session_id == self.session_id)
                .order_by(col(DialogueMessageORM.timestamp).desc())
            )
            results = await db.exec(statement)
    ...
    async def add_card_recommendation_result(
        self, result: ChildCardRecommendationResult
    ):
        async with self.get_sessionmaker() as db:
            async with db.begin():
                db.add(
                    ChildCardRecommendationResultORM.from_data_model(
                        self.session_id, result
                    )
                )

    async def add_parent_guide_recommendation_result(
        self, result: ParentGuideRecommendationResult
    ):
        async with self.get_sessionmaker() as db:
            async with db.begin():
                db.add(
                    ParentGuideRecommendationResultORM.from_data_model(
                        self.session_id, result
                    )
                )

    async def get_card_recommendation_result(
        self, recommendation_id: str
    ) -> ChildCardRecommendationResult | None:
        async with self.get_sessionmaker() as db:
    ...
    async def get_parent_guide_recommendation_result(
        self, recommendation_id: str
    ) -> ParentGuideRecommendationResult | None:
        async with self.get_sessionmaker() as db:
    ...
    async def add_parent_example_message(self, message: ParentExampleMessage):
        async with self.get_sessionmaker() as db:
            async with db.begin():
                db.add(
                    ParentExampleMessageORM.from_data_model(self.session_id, message)
                )

    async def get_parent_example_message(
        self, recommendation_id: str, guide_id: str
    ) -> ParentExampleMessage | None:
        async with self.get_sessionmaker() as db:
            statement = (
                select(ParentExampleMessageORM)
    ...
    async def add_card_selection(self, selection: InterimCardSelection):
        async with self.get_sessionmaker() as db:
            async with db.begin():
                db.add(
                    InterimCardSelectionORM.from_data_model(self.session_id, selection)
                )

    async def __get_latest_model(
        self,
        db: AsyncSession,
        model: type[SessionIdMixin],
        timestamp_column: Any | None = None,
        turn_id: Any | None = None,
    ) -> SessionIdMixin | None:
        if timestamp_column is None and hasattr(model, "timestamp"):
            timestamp_column = model.timestamp

        statement = select(model).where(model.session_id == self.session_id)

        if turn_id is not None and hasattr(model, "turn_id"):
    ...
    async def get_latest_card_selection(
        self, turn_id=None
    ) -> InterimCardSelection | None:
        async with self.get_sessionmaker() as db:
            d = await self.__get_latest_model(
                db, InterimCardSelectionORM, turn_id=turn_id
            )
    ...
    async def get_latest_parent_guide_recommendation(
        self, turn_id=None
    ) -> ParentGuideRecommendationResult | None:
        async with self.get_sessionmaker() as db:
            d = await self.__get_latest_model(
                db, ParentGuideRecommendationResultORM, turn_id=turn_id
            )
    ...
    async def get_latest_child_card_recommendation(
        self, turn_id=None
    ) -> ChildCardRecommendationResult | None:
        async with self.get_sessionmaker() as db:
            d = await self.__get_latest_model(
                db, ChildCardRecommendationResultORM, turn_id=turn_id
            )
        ...
        async with self.get_sessionmaker() as db:
            async with db.begin():
                for model in [
                    ...
                    rows = await db.exec(
                        select(model).where(model.session_id == self.session_id)
                    )
                    for row in rows:
                        await db.delete(row)
        ...
        async with self.get_sessionmaker() as db:
            async with db.begin():
                ...
                if orig_orm is None:
                    ...
                    statement = (
                        update(SessionORM)
                        .where(col(SessionORM.id) == info.id)
                        .values(
                            **SessionORM.from_data_model(info).model_dump(
                                exclude={"id", "dyad_id"}
                            )
                        )
                    )
        ...
        async with self.get_sessionmaker() as db:
            async with db.begin():
                ...
                if orig_orm is not None:
                    # Update
                    statement = (
                        update(DialogueTurnORM)
                        .where(col(DialogueTurnORM.id) == turn.id)
    ...
    async def get_latest_turn(self) -> DialogueTurn | None:
        async with self.get_sessionmaker() as db:
            d = await self.__get_latest_model(
                db, DialogueTurnORM, DialogueTurnORM.started_timestamp
            )
