            ...
            forEachChildCardAdapters((category, adapter) => {
                ...
                try {
                    unmatchedElements.forEach(cardInfo => {
                        const firstEmptyIndex = reorderedNewCardInfos.findIndex(elm => elm == null)

                        if (firstEmptyIndex >= 0) {
                            reorderedNewCardInfos[firstEmptyIndex] = cardInfo
                        } else throw Error("There must be an empty index.")
                    })
                    adapter.setAll(state.childCardEntityStateByCategory[category], reorderedNewCardInfos)
                } catch (ex) {
                    console.log(ex)
                    adapter.setAll(state.childCardEntityStateByCategory[category], newCardInfos)
                }
            })
