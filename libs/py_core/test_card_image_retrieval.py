...
from py_core.system.model import CardInfo, CardCategory, ChildGender, ParentType, UserLocale
...
async def routine(matcher: CardImageMatcher):
    ...
    while True:
        ...
        matches = await matcher.match_card_images([CardInfo(recommendation_id="", label=kw, label_localized=kwl, category=CardCategory.Topic) for kw, kwl in zip(keyword.split(","), keyword_localized.split(","))], parent_type=ParentType.Father, child_gender=ChildGender.Boy, locale=UserLocale.Korean)
