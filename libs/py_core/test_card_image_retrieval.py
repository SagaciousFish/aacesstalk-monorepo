...
async def routine(matcher: CardImageMatcher):
    ...
    while True:
        ...
        matches = await matcher.match_card_images(
            [
                CardInfo(
                    recommendation_id="",
                    label=kw,
                ...
                for kw, kwl in zip(keyword.split(","), keyword_localized.split(","))
            ],
            parent_type=ParentType.Father,
            child_gender=ChildGender.Boy,
            locale=UserLocale.SimplifiedChinese,
        )

        for m in matches:
            print(
                await matcher.get_card_image_filepath(
                    m.type,
                    m.image_id,
                    parent_type=ParentType.Father,
                    child_gender=ChildGender.Boy,
