...
class CardImageMatcher:
    ...
    async def match_card_images(self, card_info_list: list[CardInfo], parent_type: ParentType) -> list[CardImageMatching]:
        ...
        db_card_image_infos: list[list[CardImageInfo]] = await to_thread(self.__db_retriever.query_nearest_card_image_infos, [c.label for i, c in enumerate(card_info_list) if result[i] is None])
