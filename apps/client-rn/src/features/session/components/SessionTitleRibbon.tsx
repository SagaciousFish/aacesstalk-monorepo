...
const styles = StyleSheet.create({
  leftRibbon: {position: 'absolute', bottom: -14, left: -45},
  rightRibbon: {position: 'absolute', bottom: -14, right: -45}
})

function RibbonEnd(props:{fgColor?: string, direction: "left"|"right"}) {
  return (
    <Svg
      width={70} height={54}
      viewBox="0 0 78 58"
      fill="none"
      style={props.direction == 'left' ? styles.leftRibbon : styles.rightRibbon}
    >
...
export const SessionTitleRibbon = (props: {
    ...
    return <View className={`${props.containerClassName} bg-red-100`}>
      <RibbonEnd direction='left' fgColor={colors[`topic${props.category}`].ribbon}/>
      <RibbonEnd direction='right' fgColor={colors[`topic${props.category}`].ribbon}/>
