...
async def get_dyad_by_id(dyad_id: str, db: Annotated[AsyncSession, Depends(with_db_session)]):
    result = await db.exec(
        (
            select(DyadORM, DyadLoginCode)
            .where(DyadORM.id == dyad_id)
            .where(DyadLoginCode.active)
            .where(DyadLoginCode.dyad_id == DyadORM.id)
        )
    )
    row = result.first()
    if row is None:
        raise HTTPException(status_code=404, detail="NoSuchDyad")
    orm, logincode = row
...
async def _create_dyad(
    args: DyadCreateArgs, db: Annotated[AsyncSession, Depends(with_db_session)]
):
...
async def _get_free_topic_image(
    detail_id: str,
    user_storage: Annotated[UserStorage, Depends(get_user_storage_with_id)],
):
...
async def _create_free_topic_detail(
    dyad_id: str,
    ...
    if image is not None:
        ...
        local_file_path = path.join(
            AACessTalkConfig.get_free_topic_image_dir_path(dyad_id, True),
            local_filename,
        )
...
async def _update_free_topic_detail(
    dyad_id: str,
    detail_id: str,
    db: Annotated[AsyncSession, Depends(with_db_session)],
    topic: Union[str, None] = Form(None),
    description: Union[str, None] = Form(None),
    remove_image: Union[bool, None] = Form(None),
    image: Union[UploadFile, None] = None,
):
    ...
    if orm is not None:
        if image is not None:
            ...
            if orm.topic_image_filename is not None:
                image_path = path.join(
                    AACessTalkConfig.get_free_topic_image_dir_path(dyad_id, True),
                    orm.topic_image_filename,
                )
                remove(image_path)
...
async def _get_sessions(
    dyad_id: str, db: Annotated[AsyncSession, Depends(with_db_session)]
):
...
async def _delete_session(
    dyad_id: str, session_id: str, db: Annotated[AsyncSession, Depends(with_db_session)]
):
...
@router.get(
    "/{dyad_id}/dialogues/{session_id}/{turn_id}/audio", response_class=FileResponse
)
async def _get_turn_audio(
    dyad_id: str,
    session_id: str,
    turn_id: str,
    db: Annotated[AsyncSession, Depends(with_db_session)],
    ...
    if turn_info is not None:
        if turn_info.role == DialogueRole.Parent:
            if turn_info.audio_filename is not None:
                file_path = path.join(
                    AACessTalkConfig.get_turn_audio_recording_dir_path(dyad_id, False),
                    turn_info.audio_filename,
                )
                return FileResponse(
                    file_path, filename=turn_info.audio_filename, media_type="audio/m4a"
                )
            else:
...
async def _get_user_defined_cards(
    dyad_id: str,
    user_storage: Annotated[UserStorage, Depends(get_user_storage_with_id)],
):
...
async def _add_user_defined_card(
    dyad_id: str,
    image: UploadFile,
    label_localized: Annotated[str, Form()],
    category: Annotated[CardCategory, Form()],
    user_storage: Annotated[UserStorage, Depends(get_user_storage_with_id)],
    label: Union[str, None] = Form(None),
):
    info = UserDefinedCardInfoORM(
        dyad_id=dyad_id,
        label=label,
    ...
    filepath = path.join(
        AACessTalkConfig.get_user_defined_card_dir_path(dyad_id, True), filename
    )
...
async def _get_custom_card_image(
    dyad_id: str,
    card_id: str,
    user_storage: Annotated[UserStorage, Depends(get_user_storage_with_id)],
):
