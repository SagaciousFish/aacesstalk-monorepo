...
async def _get_session_info(session_id: str, dyad: Annotated[Dyad, Depends(get_signed_in_dyad_orm)], db: Annotated[AsyncSession, Depends(with_db_session)]) -> Session | None:
...
async def _abort_session(session_id: str, dyad: Annotated[Dyad, Depends(get_signed_in_dyad_orm)],
...
async def _end_session(session_id: str, dyad: Annotated[Dyad, Depends(get_signed_in_dyad_orm)],
                      db: Annotated[AsyncSession, Depends(with_db_session)]):
    try:
