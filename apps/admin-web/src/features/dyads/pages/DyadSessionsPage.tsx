        ...
        if (dyadId != null && props.sessionId != null && props.turnId != null && token != null) {
            ...
            try {
                const result = await Http.axios.get(url, {
                    headers: {
                        ...await Http.getSignedInHeaders(token),
                        'Content-Type': 'audio/*'
                    },
                    responseType: 'blob'
                })
    ...
    const timelineData = useMemo(() => {
        if (dialogue != null) {
            return dialogue.dialogue.map(message => {
                ...
                return {
                    color: message.role == DialogueRole.Parent ? 'blue' : 'green',
                    children: <div className="flex flex-row items-baseline">
                        <span className="font-bold mr-2">{
                            message.role == DialogueRole.Parent ? "Parent" : "Child"
                        }:
                        </span>
                        {content}
                    </div>
...
export const DyadSessionsPage = () => {
    ...
    const collapsableItems: CollapseProps['items'] = useMemo(() => {
        return sessions.map(session => {
            ...
            return {
                key: session.id,
                // 1. LABEL: Contains only the data columns.
                // Uses fixed widths to ensure perfect vertical alignment across rows.
                label: (
                    <div className="grid grid-cols-[110px_100px_140px_120px_80px] gap-4 items-center select-none">

                        {/* Date */}
                        <div>
                            <span className="bg-slate-300 p-1 px-1.5 rounded-md text-sm whitespace-nowrap">
                                {dateString}
                            </span>
                        </div>
                        ...
                        <div className="flex items-center">
                            <span className={`${topicColor} p-1 px-1.5 rounded-md capitalize text-sm truncate w-full text-center`} title={topicString}>
                                {topicString}
                            </span>
                        </div>

                        {/* Status */}
                        <div className={`${session.status === SessionStatus.Conversation ? "text-red-400" : "text-slate-600"} capitalize font-semibold text-sm truncate`} title={session.status}>
                            {session.status}
                        </div>

                        {/* Turns */}
                        <div className="text-sm text-gray-500 whitespace-nowrap">
                            {session.num_turns} turn(s)
                        </div>
                    </div>
                ),
                // 2. EXTRA: Native Ant Design prop for right-aligned actions.
                // This ensures the button always sticks to the right edge and never overlaps data.
                extra: (
                    <Button
                        danger
                        size="small"
                        onClick={(ev) => {
                // Stop propagation prevents the collapse from opening when deleting
                            ev.stopPropagation()
